...
function runMonthEndReminder() {
  ...
  const alreadyQueued = getDb().prepare("SELECT id FROM notification_jobs WHERE notification_type = 'month_end_reminder' AND date(created_at) = date('now') LIMIT 1").get();
  if (alreadyQueued) return;
  queueMonthEndReminders({
...
export function startNotificationScheduler() {
  ...
  runMonthEndReminder();
  const timer = setInterval(runMonthEndReminder, 60 * 60 * 1000);
  timer.unref?.();
}
