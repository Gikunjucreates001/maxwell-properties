...
export function createApproval({ requestedBy, entityType, entityId = null, action, payload, reason }) {
  const db = getDb();
...
function ensureProperty(db, propertyId) {
  const property = db.prepare('SELECT * FROM properties WHERE id = ?').get(propertyId);
  if (!property) throw new Error('Selected property was not found');
  return property;
}

function ensureAvailableUnit(db, propertyId, unitId, excludeTenantId = null) {
  ...
  const occupied = excludeTenantId == null
    ? db.prepare("SELECT id FROM tenants WHERE unit_id = ? AND status = 'active'").get(unit.id)
    : db.prepare("SELECT id FROM tenants WHERE unit_id = ? AND status = 'active' AND id != ?").get(unit.id, excludeTenantId);
  if (occupied) {
    throw new Error('Selected house unit is already occupied');
  }
  return unit;
}

function applyProperty(db, action, payload, entityId) {
  if (action === 'create') {
    const result = db.prepare(`
  ...
  if (action === 'delete') {
    const property = ensureProperty(db, entityId);
    ...
    db.prepare('DELETE FROM properties WHERE id = ?').run(entityId);
    return entityId;
  }
  ensureProperty(db, entityId);
  db.prepare(`
    UPDATE properties
...
function applyUnit(db, action, payload, entityId) {
  if (action === 'create') {
    const property = ensureProperty(db, payload.property_id);
  ...
  const unit = db.prepare('SELECT * FROM units WHERE id = ?').get(entityId);
  if (!unit) throw new Error('House unit not found');
  if (action === 'delete') {
    if (db.prepare("SELECT id FROM tenants WHERE unit_id = ? AND status = 'active'").get(entityId)) throw new Error('Cannot delete an occupied house unit');
    if (db.prepare('SELECT id FROM expenses WHERE unit_id = ?').get(entityId)) throw new Error('Cannot delete a house unit with expense history');
    db.prepare('DELETE FROM units WHERE id = ?').run(entityId);
    return entityId;
  }
  ensureProperty(db, payload.property_id);
  if (payload.status === 'maintenance' && db.prepare("SELECT id FROM tenants WHERE unit_id = ? AND status = 'active'").get(entityId)) {
...
function applyTenant(db, action, payload, entityId) {
  if (action === 'delete') {
    if (db.prepare('SELECT id FROM payments WHERE tenant_id = ?').get(entityId)) throw new Error('Cannot delete a tenant with payment history');
    db.prepare('DELETE FROM tenants WHERE id = ?').run(entityId);
    return entityId;
  }
  ...
  if (isApartmentProperty(property.type)) {
    ...
    const unit = ensureAvailableUnit(db, payload.property_id, payload.unit_id, action === 'update' ? entityId : null);
  ...
  const current = db.prepare('SELECT id FROM tenants WHERE id = ?').get(entityId);
...
function applyExpense(db, action, payload, entityId, reviewerId) {
  if (action === 'delete') {
    db.prepare('DELETE FROM expenses WHERE id = ?').run(entityId);
    return entityId;
  }
  ensureProperty(db, payload.property_id);
  if (payload.unit_id) db.prepare('SELECT id FROM units WHERE id = ? AND property_id = ?').get(payload.unit_id, payload.property_id) || (() => { throw new Error('Selected house unit was not found'); })();
  if (action === 'create') {
    const result = db.prepare(`
...
function applyIssue(db, action, payload, entityId, reviewerId) {
  if (action === 'delete') {
    db.prepare('DELETE FROM issues WHERE id = ?').run(entityId);
    return entityId;
  }
  const property = ensureProperty(db, payload.property_id);
  if (isApartmentProperty(property.type)) {
    if (!payload.unit_id) throw new Error('Apartment issues must specify a house unit');
    db.prepare('SELECT id FROM units WHERE id = ? AND property_id = ?').get(payload.unit_id, payload.property_id) || (() => { throw new Error('Selected house unit was not found'); })();
  }
  ...
  const issue = db.prepare('SELECT * FROM issues WHERE id = ?').get(entityId);
  ...
  if (issue.expense_id) {
    if (repairCost > 0) db.prepare('UPDATE expenses SET property_id = ?, unit_id = ?, description = ?, amount = ?, expense_date = ?, notes = ? WHERE id = ?').run(payload.property_id, payload.unit_id || null, `Repair: ${payload.title}`, repairCost, payload.reported_date, payload.notes, issue.expense_id);
    else db.prepare('DELETE FROM expenses WHERE id = ?').run(issue.expense_id);
    ...
    db.prepare('UPDATE issues SET expense_id = ? WHERE id = ?').run(expense.lastInsertRowid, entityId);
  }
  return entityId;
}

function applyPayment(db, action, payload, entityId) {
  const payment = db.prepare('SELECT * FROM payments WHERE id = ?').get(entityId);
  if (action === 'delete') {
    if (!payment) throw new Error('Payment not found');
    db.prepare('DELETE FROM payments WHERE id = ?').run(entityId);
    return entityId;
  }
  ...
  if (!db.prepare('SELECT id FROM properties WHERE id = ?').get(payload.property_id)) throw new Error('Selected property was not found');
  const tenant = db.prepare('SELECT id, property_id FROM tenants WHERE id = ?').get(payload.tenant_id);
...
export function executeApprovalInTransaction(db, approval, reviewerId) {
...
export function executeApproval(approval, reviewerId) {
  const db = getDb();
  return db.transaction(() => executeApprovalInTransaction(db, approval, reviewerId))();
}
