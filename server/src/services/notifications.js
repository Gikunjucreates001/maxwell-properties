...
async function queueJob({ tenantId, channel, notificationType, recipient, subject, body, createdBy }) {
...
async function deliverNotification(jobId) {
  ...
  try {
    ...
    await db.prepare("UPDATE notification_jobs SET status = 'sent', sent_at = CURRENT_TIMESTAMP, error = NULL WHERE id = ?").run(jobId);
  } catch (error) {
    await db.prepare("UPDATE notification_jobs SET status = 'failed', error = ? WHERE id = ?").run(error.message, jobId);
  }
}

export async function queueTenantMessage({ tenant, notificationType, subject, body, createdBy }) {
  const ids = [];
  if (tenant.email) ids.push(await queueJob({ tenantId: tenant.id, channel: 'email', notificationType, recipient: tenant.email, subject, body, createdBy }));
  if (tenant.phone) ids.push(await queueJob({ tenantId: tenant.id, channel: 'sms', notificationType, recipient: tenant.phone, subject, body, createdBy }));
  return ids.filter(Boolean);
}

async function getTenantCommunicationRecord(tenantId) {
...
export async function queuePaymentReceipts(paymentId, createdBy) {
  const db = getDb();
  ...
  const ids = await queueTenantMessage({ tenant: { id: payment.tenant_id, email: payment.email, phone: payment.phone }, notificationType: 'payment_receipt', subject: 'Payment receipt - Maxwell Properties', body, createdBy });
  if (ids.length) await db.prepare('UPDATE payments SET receipt_notifications_sent_at = CURRENT_TIMESTAMP WHERE id = ? AND receipt_notifications_sent_at IS NULL').run(paymentId);
  return ids;
}

export async function queueOnboardingIfEligible(tenantId, createdBy) {
  const db = getDb();
  const tenant = await getTenantCommunicationRecord(tenantId);
  if (!tenant || tenant.onboarding_sent_at) return [];
  if (!isApartmentProperty(tenant.property_type)) return [];
  const hasDeposit = await db.prepare("SELECT id FROM payments WHERE tenant_id = ? AND payment_type = 'deposit' AND status = 'paid'").get(tenantId);
  const hasFirstRent = await db.prepare("SELECT id FROM payments WHERE tenant_id = ? AND payment_type = 'rent' AND status = 'paid'").get(tenantId);
  ...
  const ids = await queueTenantMessage({ tenant, notificationType: 'onboarding', subject: `Welcome to ${tenant.property_name}`, body, createdBy });
  if (ids.length) await db.prepare('UPDATE tenants SET onboarding_sent_at = CURRENT_TIMESTAMP WHERE id = ? AND onboarding_sent_at IS NULL').run(tenantId);
  return ids;
}

export async function queueOverdueMessages({ message, createdBy }) {
  const db = getDb();
  ...
  for (const tenant of tenants) {
    ids.push(...await queueTenantMessage({ tenant, notificationType: 'overdue_reminder', subject: 'Overdue payment reminder - Maxwell Properties', body: `Hello ${tenant.name},\n\n${message}\n\nProperty: ${tenant.property_name}\nPlease contact the property team if you need assistance.`, createdBy }));
  }
  return { tenantCount: tenants.length, jobIds: ids };
}

export async function queueMonthEndReminders({ message, createdBy }) {
  const db = getDb();
  const tenants = await db.prepare(`
  ...
  const ids = [];
  for (const tenant of tenants) {
    ids.push(...await queueTenantMessage({ tenant, notificationType: 'month_end_reminder', subject: 'Monthly payment reminder - Maxwell Properties', body: `Hello ${tenant.name},\n\n${message}\n\nProperty: ${tenant.property_name}\nThank you,\nMaxwell Properties`, createdBy }));
  }
  return { tenantCount: tenants.length, jobIds: ids };
}
