...
function validateExpense(db, payload) {
  ...
  if (payload.unit_id) {
    ...
    if (!db.prepare('SELECT id FROM units WHERE id = ? AND property_id = ?').get(payload.unit_id, payload.property_id)) return 'Selected House ID was not found';
  }
  return null;
}
...
function saveExpense(db, payload, id = null, createdBy) {
  if (id == null) {
...
router.get('/:id', (req, res) => {
  try {
    const expense = getDb().prepare(`${expenseSelect} WHERE e.id = ?`).get(req.params.id);
...
router.post('/', (req, res) => {
  try {
    ...
    const validationError = validateExpense(db, payload);
    if (validationError) return res.status(400).json({ success: false, error: validationError });
    if (req.user.role === 'manager') {
      const approval = createApproval({ requestedBy: req.user.id, entityType: 'expense', action: 'create', payload, reason: `Log ${payload.category.replace('_', ' ')} expense` });
      return res.status(202).json({ success: true, pending: true, data: approval, message: 'Expense submitted for admin approval' });
    }
    const id = saveExpense(db, payload, null, req.user.id);
    res.status(201).json({ success: true, data: db.prepare(`${expenseSelect} WHERE e.id = ?`).get(id) });
  } catch (error) {
...
router.put('/:id', (req, res) => {
  try {
    ...
    if (validationError) return res.status(400).json({ success: false, error: validationError });
    if (req.user.role === 'manager') {
      const approval = createApproval({ requestedBy: req.user.id, entityType: 'expense', entityId: current.id, action: 'update', payload, reason: `Update expense ${current.description}` });
      return res.status(202).json({ success: true, pending: true, data: approval, message: 'Expense update submitted for admin approval' });
    }
    const id = saveExpense(db, payload, current.id, req.user.id);
    res.json({ success: true, data: db.prepare(`${expenseSelect} WHERE e.id = ?`).get(id) });
  } catch (error) {
...
router.delete('/:id', (req, res) => {
  try {
    ...
    if (!expense) return res.status(404).json({ success: false, error: 'Expense not found' });
    if (req.user.role === 'manager') {
      const approval = createApproval({ requestedBy: req.user.id, entityType: 'expense', entityId: expense.id, action: 'delete', payload: {}, reason: `Delete expense ${expense.description}` });
